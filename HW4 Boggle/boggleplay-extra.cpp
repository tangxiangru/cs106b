/* Boggle Play Extra file: determines CLI for game
 * Creator: Cristobal Sciutto
 * Section Leader: Shirin Salehi
 *
 * Extra Features: support for 5x5 Boggle Board
 * Look at lines: getSize(): 28 - 26, Reset GUI: 184, Substitution of 4 for size variable elsewhere
 *
...
/***************************************************************************
 Functions: getSize()
 ---------------------------------------------------------------------------
 Requests the size of the board to the user, making sure it is either 4 or 5
 ***************************************************************************/

int getSize() {
    int size;
    while (true) {
        size = getInteger("What size should the board be? [4 or 5] ");
        if (size > 3 && size < 7) break;
        else cout << "Please enter a valid size" << endl;
    }
    return size;
}
...
bool checkInput(string boardText, int size) {
    if ((int)boardText.size() != size*size) return false;
...
string inputForBoard(int size) {
    string boardText;
	while (true) {
        boardText = getLine("Type the" + integerToString(size*size) + "letters to appear on the board: ");
        if (checkInput(boardText, size)) break;
        else cout << "That is not a valid " + integerToString(size*size) +"-letter board string. Try again." << endl;
	}
...
void playOneGame(Lexicon& dictionary) {
    int size = getSize();
    string boardText = getYesOrNo("Do you want to generate a random board? ") ? "" : inputForBoard(size);
    Boggle myBoard(dictionary, boardText, size);
	string prompt = "It's your turn!";

    // Re-initializing the GUI and labeling all cubes
    BoggleGUI::close();
